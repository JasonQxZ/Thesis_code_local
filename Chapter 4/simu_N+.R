...
Gen_Data_Simulation <- function(simulation_time, P , k , family, causal_feature_pos,causal_features_coef,Num_simu, Methods,Metrics){

  N = 150+ 65*simulation_time
  ...
  for(j in 1:Num_simu){
    ...
    for(i in 1:length(Methods)){
      ...
      Table_test_error[i,j] <- sum(abs(Data_test_y-predict(model,newdata,type = "response")))
...
# Parallel execution
Results <- mclapply(simulation_times, function(x) Gen_Data_Simulation(x, P, k, family, causal_feature_pos, causal_features_coef, Num_simu, Methods, Metrics)s)

# Optional: Name the list elements based on simulation times for clarity
names(Results) <- paste("Simulation", simulation_times)

save(Results, file = "2.23_increasingN_4in1plots.RData")
