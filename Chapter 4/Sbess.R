...
Sbess<-function(Y, X, k, family=gaussian(), d = k*2, m = 6,
                    max_iter = 500, tol = 10^(-3), U = 1,
                    U_rate= 0.5, lasso_initial = F, Strategy = 4 
                ){
  ...
  repeat{
    ...
    ID_out <- cbind(ID_out, IHT_res_out$ID_None_s)
    
    Beta_out <- cbind(Beta_out, IHT_res_out$Beta_s)
    
    llh_out <- c(llh_out, IHT_res_out$LH)
