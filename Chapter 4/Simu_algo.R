Run_method <- function(METD , k = NULL , family , Data ){
  ...
  switch(METD, 
         ...
         SIS_s = run_SIS_select(x = Data$X,y = Data$Y, family = family),
         
         lasso_cv = run_cvglment( x = Data$X, y = Data$Y, family = family),
         
         ISIS = run_SIS(x = Data$X,y = Data$Y,k = k, family = family, iter =T),
         
         Tilting = run_tilting(x = Data$X,y = Data$Y, k = k, family = family),
        
         abess_meta  = run_abess_meta( x = Data$X, y = Data$Y, family = family))
...
run_SIS_select <-  function(x , y,family , iter = FALSE){
  
  SIS(x , y , family = family$family ,iter = iter)$ix
...
run_abess_meta <-  function(x , y,family , k = NULL ){
  
  abess_fit <- abess(x = x, y = y, family = family$family , support.size = k)
  
  ind <- extract(abess_fit)$support.vars
  
  ind <- (1:1071)[colnames(x) %in% ind]
