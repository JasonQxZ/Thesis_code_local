...
ebic <- function(Beta_s = NULL ,Y, X, family , s = NULL,  gamma_ = 0.6){
  ...
  if(is.null(s)){
    ...
    ebic <- -2*lh( Y, X, Beta_s, family = family ) + k * log(n) +2* k*gamma_ * log(p)
    ...
    Beta_s[s] <-  coefficients(glm.fit(x = X[,s],y = Y, family = family))
...
IHT_w_ucheck <-function(Beta_s, k , Y, X, family,U, U_rate, tol, max_iter,p){
  ...
  repeat{
    ...
    LH[i] <- lh( Y, X[,sindex], Beta_t[sindex], family = family )
    ...
    LH_last <- lh( Y, Xs_s, coef_None_s, family = family )
