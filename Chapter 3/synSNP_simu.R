...
Methods = list("BANS",'alpha_investing',"Iter_SIS","Iter_SMLE","Saola_z")

Metrics = list( "trainloss", "testloss", "model_size","bic","time","psr")
...
x <- as.matrix(synSNP[,2:10032],nrow=800)

y <- as.vector(synSNP[,1])
...
for( m in 1:num_Simu){
  ...
  train_index <- sample(1:dim(x)[1],floor(0.6*dim(x)[1]),replace = FALSE)
  
  test_index <- (1:dim(x)[1])[!(1:dim(x)[1]) %in% train_index]
  
  train_size <- length(train_index)
  
  Data<-list(Y = y,X = x, coef_true= NULL,subset_true=(1:10032)[names(synSNP) %in% c('rs2967291', 'rs1534941', 'rs112915930', 'rs6132052','rs12608528','rs11157211')]-1)
