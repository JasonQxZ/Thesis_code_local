...
boxplot(PSR_DIFF,cex.lab=2, ylab ="PSR", boxfill = NA, border = NA) #invisible boxes - only axes and plot area
boxplot(PSR_DIFF, xaxt = "n", add = TRUE, boxfill="red", 
        boxwex=0.25, at = 1:ncol(PSR_DIFF) - 0.15) 
abline(h=0,lty=2)

#shift these left by -0.15
boxplot(TEST_DIFF,cex.lab=2, ylab ="Pred. Err.", boxfill = NA, border = NA)
boxplot(TEST_DIFF, xaxt = "n", add = T, boxfill="blue",
        boxwex=0.25, at = 1:ncol(TEST_DIFF) + 0.15) #shift to the right by +0.15
abline(h=0,lty=2)
print(Result)
