...
for( m in 1:num_Simu){
  ...
  train_index <- sample(1:dim(x)[1],floor(0.7*dim(x)[1]),replace = FALSE)
  ...
  Sensitivity <- matrix(0,nrow = num_methods, ncol =num_iters)
  
  Specificity <-  matrix(0,nrow = num_methods, ncol =num_iters)
  ...
  for( i in 1:num_methods){
    ...
    for(j in 1:num_iters){
      ...
      Train_loss[i,j] <-sum( abs( predict(model,type = "response") - y[train_index]))/train_size
      
      newdata <- data.frame( X = X[test_index,Test_Result@shuffle_order[Index_set[[j]]]], Y = y[test_index])
      
      y_test <- y[test_index]
      
      Test_Loss[i,j] <- sum( abs( predict(model,newdata,type = "response") - y_test))/(187-train_size)
      
      y_pred <- as.vector(predict(model,newdata,type = "response")> threshold)
      
      Positive_sample_index <- y_test == 1
      
      True_Positive<- y_pred[Positive_sample_index] == y_test[Positive_sample_index]
      
      Sensitivity[i,j] <- sum(True_Positive)/length(Positive_sample_index)
      
      Negative_sample_index <- y_test == 0
      
      True_Negative <- y_pred[Negative_sample_index] == y_test[Negative_sample_index]
      
      Specificity[i,j] <- sum(True_Negative)/length(True_Negative) 
  ...
  Table_r_Sensitivity[,m] = Sensitivity[,num_iters]
  Table_r_Specificity[,m] = Specificity[,num_iters]
}
