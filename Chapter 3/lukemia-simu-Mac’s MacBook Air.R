...
load("/Users/mac/Downloads/propOverlap/data/leukaemia.rda")

y = leukaemia[7130,]-1

names(y) <- NULL

x = standardize(t(matrix(leukaemia[1:7129,],nrow = 7129)))
...
for( m in 1:num_Simu){
  ...
  train_index <- sort(sample(1:dim(x)[1],60,replace = FALSE))
  ...
  for( i in 1:num_methods){
    ...
    for(j in 1:num_iters){
      ...
      newdata <- data.frame( X = X[test_index,Test_Result@shuffle_order[Index_set[[j]]]])
      
      y_test <- y[test_index]
      
      Test_Loss[i,j] <-  - (y_test%*%log(predict(model,newdata,type = "response"))+(1-y_test)%*%log(1-predict(model,newdata,type = "response")))/(N-train_size)
...
save(Result, file = "simu_lukemia.rds")
